...
const createWeatherCard=(cityName,weatherItem,index)=>{
    if(index === 0) {
        return `<div class="details">
                    ...
                    <h4>Temperature:${(weatherItem.main.temp - 273.15).toFixed(2)}°C</h3>
        ...
        return `<li class="card">
                ...
                <h4>Temp:${(weatherItem.main.temp - 273.15).toFixed(2)}°C</h3>
...
const getWeatherDetails = (cityName, lat, lon) =>{
    ...
    fetch(WEATHER_API_URL).then(res => res.json()).then(data =>{
        ...
        alert("An Error occured in weather")
    })
}
...
const getCity=()=>{
    ...
    getWeatherDetails(name, lat, lon )
        ...
        alert("An Error occured in location")
    })
}
    ...
    navigator.geolocation.getCurrentPosition(
        ...
        error=>{
            if(error.code === error.PERMISSION_DENIED){
                alert("Location denied.Please reset the location permission to grant to access again.")
            }
        }
    )}
